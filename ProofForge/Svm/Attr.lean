...
/-!
SVM extension of the shared ProofForge attribute surface: packed Solana wire-entry
adapters (`pf_svm_raw*`). The base `pf_entry` / `pf_inline` / `pf_boundary` attributes
live in the `proofforge-common` package; this module reopens `ProofForge.Attr` and adds
the target-owned layer.
-/
