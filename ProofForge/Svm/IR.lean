    ...
    | .invoke programIx metas data seeds bump =>
        let metaCanon := String.intercalate "," <| metas.toList.map fun entry =>
          ...
          let policy :=
            ...
            | some .token2022MemoTransferAccount => "~t22memo"
            | some .token2022TransferHookAccount => "~t22thacct"
            | some .token2022TransferHookMint => "~t22thmint"
            | some .token2022DefaultAccountStateMint => "~t22das"
            | some .token2022MetadataPointerMint => "~t22mdptr"
            | some .token2022GroupPointerMint => "~t22gptr"
            | some .token2022GroupMemberPointerMint => "~t22gmptr"
            | some .token2022TokenGroupMint => "~t22tgrp"
            | some .token2022TokenGroupMemberMint => "~t22tgmem"
            | none => ""
