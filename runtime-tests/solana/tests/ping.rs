...
fn ping_so() -> PathBuf {
    PathBuf::from(env::var("PF_PING_SO").unwrap_or_else(|_| {
        format!(
            "{}/build/sbpf/Ping.so",
...
fn harness() -> (Pubkey, Pubkey, Mollusk) {
    ...
    let elf = fs::read(ping_so()).unwrap_or_else(|e| panic!("read Ping.so: {e}"));
...
#[test]
fn ping_invokes_account_1() {
    let (program_id, callee_id, mollusk) = harness();
    let payer = Pubkey::new_unique();
    let disc = instruction_discriminator("ping", 0);
