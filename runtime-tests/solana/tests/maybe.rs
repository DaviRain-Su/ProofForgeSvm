...
fn layout_marker() -> u64 {
    let layout_sig = "2|0:slot_tag:0:8:8:u64-le|1:slot_p0:0:16:8:u64-le";
...
fn maybe_state(initialized: bool, tag: u64, payload: u64) -> Vec<u8> {
...
fn so_path() -> PathBuf {
    PathBuf::from(env::var("PF_MAYBE_SO").unwrap_or_else(|_| {
        format!(
            "{}/build/sbpf/Maybe.so",
...
fn harness() -> (Pubkey, Mollusk) {
    ...
    let elf = fs::read(so_path()).unwrap_or_else(|e| panic!("read Maybe.so: {e}"));
...
#[test]
fn set_some_writes_tag_and_payload() {
    let (program_id, mollusk) = harness();
    let state_key = Pubkey::new_unique();
    let disc = instruction_discriminator("setSome", 1);
...
#[test]
fn set_none_clears_both_leaves() {
    let (program_id, mollusk) = harness();
    let state_key = Pubkey::new_unique();
    let disc = instruction_discriminator("setNone", 0);
...
fn is_some_view() {
    let (program_id, mollusk) = harness();
    let state_key = Pubkey::new_unique();
    let disc = instruction_discriminator("isSome", 0);
    let ix = build_ix(program_id, state_key, &disc, &[], false, false);
    let account = state_account(&program_id, maybe_state(true, 1, 77));
    mollusk.process_and_validate_instruction(
        &ix,
