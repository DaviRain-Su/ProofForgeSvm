...
use {
    ...
    mollusk_svm_programs_token_2022::{
        ...
        AccountState, Mint, MintExtension, PausableConfig, TokenAccount,
    },
    sha2::{Digest, Sha256},
...
fn harness() -> (Pubkey, Mollusk) {
    let (program_id, mut mollusk) = common::harness("Token2022Pause", "PF_TOKEN2022PAUSE_SO");
...
fn pausable_mint(authority: Pubkey, paused: bool) -> Account {
    create_account_for_mint_with_extensions(
        mint_state(authority, INITIAL),
        &[MintExtension::Pausable(PausableConfig {
...
fn build_ix(program_id: Pubkey, authority: Pubkey, source: Pubkey, mint: Pubkey, dest: Pubkey) -> Instruction {
    let disc = instruction_discriminator("transferPause", 1);
...
#[test]
fn transfer_over_unpaused_mint_succeeds() {
    ...
    mollusk.process_and_validate_instruction(
        ...
        &[
            ...
            (authority, funded()),
            (source, create_account_for_token_account_with_extensions(token_state(mint, authority, INITIAL), &[])),
            (mint, pausable_mint(authority, false)),
...
#[test]
fn transfer_over_paused_mint_rejected_by_token_program() {
    let (program_id, mollusk) = harness();
    let authority = Pubkey::new_unique();
    let source = Pubkey::new_unique();
    let mint = Pubkey::new_unique();
    ...
    mollusk.process_and_validate_instruction(
        ...
        &[
            ...
            (authority, funded()),
            (source, create_account_for_token_account_with_extensions(token_state(mint, authority, INITIAL), &[])),
            (mint, pausable_mint(authority, true)),
            (dest, create_account_for_token_account_with_extensions(token_state(mint, authority, 0), &[])),
            token2022::keyed_account(),
        ],
        &[
            // Token-2022 MintPaused = TokenError discriminant 67.
            Check::err(ProgramError::Custom(67)),
