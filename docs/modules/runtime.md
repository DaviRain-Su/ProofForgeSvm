...
`Examples/Svm/NonceLife.lean` + `runtime-tests/solana/tests/nonce_life.rs`：五条 System nonce 生命周期指令全部链上验证。`openNonce` = createRentExempt 80（rent 1,447,680 经 live sysvar 计算）；`initNonce` = InitializeNonceAccount（CPI 账户序 [nonce, recent_blockhashes, rent]，数据 [u32 6][authorized]，recent sysvar 缓存需非空否则 NonceNoRecentBlockhashes = Custom 6，nonce 账户需 rent-exempt）；`reauthNonce` = AuthorizeNonceAccount（CPI [nonce, current_authority(signer)]，数据 [u32 7][new authority]，System 校验当前授权人已签名）；`withdrawNonce` = WithdrawNonceAccount（CPI [nonce, dest, recent, rent]，数据 [u32 5][lamports]，提款后余额须 ≥ rent 下限）；`upgradeNonce` = UpgradeNonceAccount（CPI [nonce]，数据 [u32 12]，nonce data 须为 Versions::Legacy(Initialized)，升级为 Current）。所有入口共享程序级 7 账户 gate。
`Examples/Svm/Token2022ExtOpen.lean` + `runtime-tests/solana/tests/token_2022_ext_open.rs`：MemoTransfer 目的地（无 memo → NoMemo = Custom 36）、TransferHook mint + TransferHookAccount 目的地（未部署 hook → fail closed）、DefaultAccountState / MetadataPointer / GroupPointer / GroupMemberPointer / TokenGroup / TokenGroupMember mint 的 transfer-neutral 准入配方。
`Svm.Runtime.accDataByteAt`：字节粒度账户数据读（绝对偏移，补 `accDataWord` 的 8 字节对齐盲区）；`Svm.Sdk.Token2022.transferFeeBasisPoints/Older/Newer`、`transferFeeMaximumFee/Older/Newer` 通过该叶子按官方绝对 TLV 偏移（older epoch 242 / max 250 / bps 258；newer epoch 260 / max 268 / bps 276）读取活动费率；`transferFeeBasisPoints`/`transferFeeMaximumFee` 按官方 `get_epoch_fee` 选择 schedule。
