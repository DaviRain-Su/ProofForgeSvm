...
`Examples/Svm/NonceLife.lean` + `runtime-tests/solana/tests/nonce_life.rs`：`System.createRentExempt 80` 创建 rent-exempt 非空账户（链上验证，出租金经 `sol_get_rent_sysvar` 计算，空间 80）。`systemInitializeNonce` / `systemWithdrawNonce` / `systemAuthorizeNonce` / `systemUpgradeNonce` 配方与 `systemAdvanceNonce` 同构；`initNonce` on-chain 验证：System `InitializeNonceAccount` 需 CPI 账户序 `[nonce, recent_blockhashes, rent]` 且 recent_blockhashes sysvar 缓存非空（否则 `NonceNoRecentBlockhashes` = Custom 6），数据 `[u32 6][authorized pubkey]`，nonce 账户需 rent-exempt。
